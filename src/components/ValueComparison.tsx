...
export default function ValueComparison() {
  const items = [
    { name: "속초 디너 팬미팅 프리패스", market: "8만원", member: "포함" },
    { name: "코요태 썸머 콘서트 프리패스", market: "15만원", member: "포함" },
    { name: "All-inclusive 팬크루즈 투어", market: "44만원", member: "포함" },
    { name: "코요태 단독 공연 VIP 관람", market: "20만원 상당", member: "포함" },
    { name: "신지 한정판 기념 주화", market: "15만원 상당", member: "포함" },
    { name: "기념 굿즈 카드 및 달력 패키지", market: "10만원 상당", member: "포함" },
  ];
    ...
    <section id="value" className="py-24 px-6 md:px-12 bg-white">
      <div className="max-w-7xl mx-auto">
        <div className="mb-16 space-y-4">
          ...
          <p className="text-slate-500">회원 가입비의 2배가 넘는 실질적인 혜택을 수치로 확인해 보세요.</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
          ...
          <div className="lg:col-span-7 bg-slate-50 rounded-2xl md:rounded-3xl overflow-hidden border border-slate-100">
              ...
              <table className="w-full text-left border-collapse min-w-[500px]">
                 <thead>
                   <tr className="border-b border-slate-200 bg-slate-100">
                     <th className="px-5 md:px-8 py-5 md:py-6 text-xs md:text-sm font-bold text-slate-500 uppercase tracking-widest">혜택 항목</th>
                     <th className="px-5 md:px-8 py-5 md:py-6 text-xs md:text-sm font-bold text-slate-500 uppercase tracking-widest text-right">일반 판매가</th>
                     <th className="px-5 md:px-8 py-5 md:py-6 text-xs md:text-sm font-bold text-blue-600 uppercase tracking-widest text-right">회원 혜택</th>
                   </tr>
                 </thead>
                 <tbody>
                   {items.map((item, i) => (
                     <tr key={i} className="border-b border-slate-100 hover:bg-blue-50/50 transition-colors">
                        <td className="px-5 md:px-8 py-4 md:py-5 text-sm font-bold text-slate-700">{item.name}</td>
                        <td className="px-5 md:px-8 py-4 md:py-5 text-xs md:text-sm text-slate-400 text-right">{item.market}</td>
                        <td className="px-5 md:px-8 py-4 md:py-5 text-sm font-black text-blue-600 text-right">
            ...
            <div className="lg:hidden p-4 bg-blue-50/50 text-center text-[10px] text-slate-400 font-bold uppercase tracking-widest">
               ← 밀어서 상세 비교 확인 →
            </div>
          </div>
          ...
          <motion.div 
            ...
            viewport={{ once: true }}
            className="lg:col-span-5 bg-blue-600 p-10 rounded-3xl flex flex-col justify-between shadow-2xl relative overflow-hidden text-white"
          >
             <div className="space-y-8 relative z-10">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex justify-between items-center pt-4 border-t border-white/10">
                    ...
                    <span className="text-5xl font-display font-black text-white tracking-tighter">{membershipFee}</span>
                  </div>
                </div>
